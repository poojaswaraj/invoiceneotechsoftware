...
<div class="row">
<h3>Edit Estimate&nbsp;-&nbsp;<?php echo $data['number']; ?></h3><hr>
...
<div class="col-lg-9">
	...
	<div class="col-md-6">
		...
		<div class="col-md-6">
			...
			<input type="email" class="form-control" name="txt_scemail" id="txt_scemail" value="<?php if(isset($_GET["id"])){ echo $data['ship_email'];} else {}?>"  placeholder="Client Email Address">
		</div>
...
<label>
	<?php 
	  if($utype=='admin')
		{
	?>
	  <a href="#" id="change" onclick="convert('<?php echo $id;?>','<?php echo $utype;?>');">+ Convert Into Proforma Invoice</a>
...
<!-- <h4>Tags</h4>
	...
	<?php 
		...
		if($template=='temp2')
		{
			 require_once"print_estimate12.php";
		}
		else{
			 require_once"print_estimate.php";
		}
	?>
	</div>
	  	 ...
	  	 else
	  	 {
	  		$("button#product_submit1").button('loading');
	           $.ajax({
						...
						type:"POST",
						url:'update_estimate_product.php',
						success: function(data)
						{
						...
						success: function(data)
						{
							...
							if(data==1)
							{
								...
								swal({
								  ...
								  window.setTimeout(function()
								    { 
								    window.location.href= "user_dashboard.php?page=edit_estimate&id=<?php echo $id;?>";
								 	} ,1500);
							...
							else if(data==3)
							{
								...
								swal({
								  ...
								  window.setTimeout(function()
								    { 
								    window.location.href= "dashboard.php?page=edit_estimate&id=<?php echo $id;?>";
								 	} ,1500);
...
// function for convert profoma invoice redirection
function convert(id,utype)
{ 
	$.ajax({
	        url:'remove_pro.php',
	        type: "POST",
	        ...
	        success: function(data)
	            {
	              ...
	              if(data==1)
	               {
						window.location.href= "dashboard.php?page=profarma_incoice&id=<?php echo $id;?>&cust_id=<?php echo $cust_id;?>"
	                	...
	                	window.location.href= "user_dashboard.php?page=profarma_incoice&id=<?php echo $id;?>&cust_id=<?php echo $cust_id; ?>"
	                } 
	                else if(data==4){
	                	window.location.href= "user_dashboard.php?page=profarma_incoice&id=<?php echo $id;?>&cust_id=<?php echo $cust_id; ?>"
	                }                     
                                  ...
                                  //alert(data);
                                    if(data==1)
                                    {
                                        ...
                                        swal({
										  	...
										  	window.setTimeout(function()
										    { 
 												window.location.href= "dashboard.php?page=estimates";	
 										 	} ,1500);
                                    }                          
                                }
                        });
