...
?>
<div class="row">
<h3>Payment Received</h3><hr>
	
<div class="col-lg-12" id="filter"> 
	<h4>Add Payment Details</h4>
	<form id="frm_payment_received" name="frm_payment_received" method="post" autocomplete="off">
		...
		<label>Bill No <span class="required_field">*</span></label>
			<input type="text" class="form-control" name="txt_bill_no" id="bill_no" placeholder="Bill No" value="<?php if(isset($rec_id)){echo $data['bill_no'];}else{} ?>" requied>
		</div>
		...
		<table class="datatable table table-striped" id="example" cellspacing="0" width="100%">
		  <thead>
			<tr>
		  	...
		  	<?php 
		  $sr_no=0;
		  		$sql = mysql_query("SELECT * FROM pay_info WHERE type='Receive'")or die(mysql_error($connection));
		  ...
		  <tbody>
			<tr id="invoice-23">
			  ...
			  <td>
			  <?php if($utype=='admin'){ ?>
			  	<a href='dashboard.php?page=payment_receive&id=<?php echo $id; ?>' title='Edit Details' data-toggle='tooltip'><span class="icon fa fa-edit"></span></a>
			  <?php }else{ ?> 
			  	<a href='user_dashboard.php?page=payment_receive&id=<?php echo $id; ?>' title='Edit Details' data-toggle='tooltip'><span class="icon fa fa-edit"></span></a>
			  <?php } ?>|
	...
	<div class="modal-dialog">
		...
		<div class="modal-content">
		  	<div class="modal-header">
			    ...
			    <h4 class="modal-title" align="center">Delete Payment Received Info</h4>
		  	</div>
...
$('form#frm_payment_received').submit(function(e){
  	...
  	if(document.pressed == 'save')
	{
	  	  $("button#btn_pay_paid").button('loading');
			        ...
			        $.ajax({
								data:$("#frm_payment_received").serialize(),
								type:"POST",
								url:'insert_pay_receive.php',
								success: function(data)
								{
						...
						success: function(data)
						{
							...
							if(data==1)
							{
								swal({
									  position: 'top-right',
	 							      type: 'success',
	  								  title: 'Pay Received  info has been Updated',
	  								  showConfirmButton: false,
	  								  timer: 1500
								  })
								  window.setTimeout(function()
								    { 
								      window.location.href= "user_dashboard.php?page=payment_receive";
								 	} ,1500);
							}
							else if(data==3)
							{
								swal({
									  position: 'top-right',
	 							      type: 'success',
	  								  title: 'Pay Receive info has been Updated',
	  								  showConfirmButton: false,
	  								  timer: 1500
								  })
								  window.setTimeout(function()
								    { 
								      window.location.href= "dashboard.php?page=payment_receive";
								 	} ,1500);
							}
							else if(data==2)
							{
