  ...
  <div class="panel panel-default">
    <div style="height: 200px; overflow-y: scroll; overflow-x: hidden;">
	  ...
	  <?php
	  	...
	  	if(empty($rec_id))
	  	{
	  	$sq = mysql_query("SELECT d.name,b.pro_name,c.mat_name,c.brand_name, a.* FROM `daily_material_requisition` a INNER JOIN new_project b ON a.pro_id=b.id INNER JOIN mst_material c ON a.mate_id=c.id INNER JOIN user_profile d ON d.id=a.requ_made_by WHERE a.id IN ($values)")or die(mysql_error($connection));
	  }else{
	  	$sq = mysql_query("SELECT d.name,b.pro_name,c.mat_name,c.brand_name, a.* FROM `daily_material_requisition` a INNER JOIN new_project b ON a.pro_id=b.id INNER JOIN mst_material c ON a.mate_id=c.id INNER JOIN user_profile d ON d.id=a.requ_made_by WHERE a.id='$rec_id'")or die(mysql_error($connection));
	  }
	...
	function get_data(id){
		...
		$.ajax({
	                ...
	                success:function(data)
	                {
	                	...
	                	$('#txt_price').val(data.grand_total);
	                	$('#select_tax').val(data.tax_value);
	                }
	            });
...
$('form#edit_myform').submit(function(e){
	...
	$.ajax({
				...
				type:"POST",
				url:'insert_purchase_material.php',
				success: function(data)
				{
					$("button#btn_save").button('reset');
					if(data==3)
