<?php
	...
	if(isset($_GET['id']))
	{
		...
		$sql = mysql_query("SELECT e.name as states,e.state_code,f.ct_name,c.address,c.comp_name,c.panno,c.gst_no,c.email,c.name,c.id as sup_id,b.pro_name, a.* FROM `tbl_debit` a LEFT JOIN mst_supplier c ON a.supp_id=c.id LEFT JOIN daily_material_requisition d ON d.id=a.supp_id LEFT JOIN new_project b ON a.po_id=b.id OR d.pro_id=b.id INNER JOIN tbl_states e ON c.state=e.id INNER JOIN city f ON f.ct_id=c.city WHERE a.id='$rec_id'")or die(mysql_error($connection));
		 ...
		 $po_no = $data['ref_po_no'];
		 $po_date = $data['ref_po_date'];
...
<div class="col-lg-3">
	<div class="col-md-12">
		...
		<input type="text" class="form-control" name="debit_no" id="debit_no" data-placeholder="Debit Note No" required=""  value="<?php echo $debit_no; ?>">
	</div>
	<div class="panel-body"></div>
	<div class="col-md-12">
		<label>Debit Note Date <span class="required_field">*</span></label>
		<input type="date" class="form-control" name="debit_date" id="debit_date" data-placeholder="Debit Note Date" required="" value="<?php echo $debit_date; ?>">
	</div>
	<div class="panel-body"></div>
	<div class="col-md-12">
    ...
    <div style="height: 200px; overflow-y: scroll; overflow-x: hidden;">
	  ...
	  <tbody>
	  	<?php 
      		...
      		$sql = mysql_query("SELECT  b.pro_name,c.mat_name,c.brand_name,a.* FROM `tbl_debit_materials` a INNER JOIN new_project b ON a.pro_id=b.id INNER JOIN mst_material c ON a.mate_id=c.id WHERE a.debit_id='$rec_id' ")or die(mysql_error($connection));
...
<?php require_once "print_debit.php"; ?>
<div class="panel-body"></div>
</div>
    ...
    <div class="modal-dialog modal-lg">
        ...
        <div class="modal-content">
            ...
            <form  method="post" id="edit_myform" enctype="multipart/formdata"> 
                ...
                <input type="hidden" name="txt_id1" id="txt_id1" value="">
				...
				<div class="col-md-12">
					...
					<label>Material Name <span class="required_field">*</span></label>
						<select name="material_names" id="mates_id" class="form-control" required="" readonly="">
					...
					<div class="col-md-6">
						...
						<textarea class="form-control" name="txt_des" id="txt_des" placeholder="Description" readonly="" ></textarea>
					</div>
				</div>
				<div class="panel-body"></div>
				...
				<div class="col-md-6">
					...
					<select name="txt_units" id="txt_units" class="form-control" required="" readonly="">
 ...
 function get_data(id){
	...
	$.ajax({
                ...
                success:function(data)
                {
	                ...
	                console.log(obj);
	                $('#txt_id1').val(obj.id);
               		$('#po_mate').val(obj.pro_id);
                    $('#mates_id').val(obj.mate_id);
            	    $('#txt_des').val(obj.description);
            	    $('#txt_units').val(obj.unit);
                    $('#txt_qtys').val(obj.qty);
            	    $('#txt_rates').val(obj.rate);
            	    $('#txt_comments').val(obj.comment);
