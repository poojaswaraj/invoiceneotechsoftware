<?php
	...
	if(isset($_GET['id']))
	{
		...
		$sql = mysql_query("SELECT * FROM mst_supplier WHERE id='$rec_id'")or die(mysql_error($connection));
...
<div class="row">
	...
	<div class="col-lg-12">
		...
		<form id="frm_supplier" name="frm_supplier" autocomplete="off" method="POST">
			...
			<div class="col-md-4">
				...
				<input type="text" class="form-control" name="txt_name" id="txt_name" placeholder="Supplier Name" value="<?php if(isset($rec_id)){echo $data['name'];}else{} ?>" style="text-transform:capitalize;" required="">
			</div>
			<div class="col-md-4">
			<label>Select Gender <span class="required_field">*</span></label>
			...
			<div class="col-md-4">
				...
				<input type="text" class="form-control" name="txt_email" id="txt_email" placeholder="Email Id" value="<?php if(isset($rec_id)){echo $data['email'];}else{} ?>">
			</div>	
			...
			<div class="col-md-4">
				...
				<input type="text" class="form-control" name="txt_adharno" id="txt_adharno" placeholder="Aadhar Number" maxLength="14" minlength="12"  value="<?php if(isset($rec_id)){echo $data['adharno'];}else{} ?>" >
				<p id="err"></p>
			</div>
			<script>
            ...
            <div class="col-md-4">
				...
				<?php 
					if(!empty($rec_id)){
					$sql1=mysql_query("SELECT a.* FROM tbl_states a INNER JOIN mst_supplier b ON a.id=b.state WHERE b.id='$rec_id'")or die(mysql_error($connection));
           	...
           	<div class="col-md-4">
				...
				<?php if(!empty($rec_id)){
					$sql1=mysql_query("SELECT a.* FROM city a INNER JOIN mst_supplier b ON a.ct_id=b.city WHERE b.id='$rec_id'")or die(mysql_error($connection));
					$row1=mysql_fetch_array($sql1);
				?>
	...
	<div class="col-lg-12" id="reportdata">
		...
		<table class="datatable table table-striped" id="example" cellspacing="0" width="100%">
		  ...
		  <tbody>
		  	<?php 
		  $sr_no=0;
		  		$sql = mysql_query("SELECT b.name as st_name,c.ct_name, a.* FROM `mst_supplier` a INNER JOIN tbl_states b ON a.state=b.id INNER JOIN city c ON a.city=c.ct_id")or die(mysql_error($connection));
		  		while($row=mysql_fetch_array($sql))
		  		{
		  			$sr_no++;
		  			$id=$row['id'];
		  ?>
			<tr id="invoice-23">
			  ...
			  <td>
			  <?php if($utype=='admin'){ ?>
			  	<a href='dashboard.php?page=supplier&id=<?php echo $id; ?>' title='Edit Details' data-toggle='tooltip'><span class="icon fa fa-edit"></span></a>
			  <?php }else{ ?> 
			  	<a href='user_dashboard.php?page=supplier&id=<?php echo $id; ?>' title='Edit Details' data-toggle='tooltip'><span class="icon fa fa-edit"></span></a>
			  <?php } ?>
	...
	<div class="modal-dialog">
		...
		<div class="modal-content">
		  	<div class="modal-header">
			    ...
			    <h4 class="modal-title" align="center">Delete Supplier</h4>
		  	</div>
          ...
          if(txt_contact.length>=10 && txt_contact.length<=15)
              	{
			        ...
			        $.ajax({
								...
								success: function(data)
								{
									...
									// else if(data==33)
									// {
									// 	$('#msg').html('Invalid Email Formate');
									// 	$('#msg').css('color','red');
								...
								success: function(data)
								{
									...
									if(data==1)
									{
										swal({
											  position: 'top-right',
			 							      type: 'success',
			  								  title: 'Supplier info has been Updated',
			  								  showConfirmButton: false,
			  								  timer: 1500
										  })
										  window.setTimeout(function()
										    { 
										      window.location.href= "user_dashboard.php?page=supplier";
										 	} ,1500);
									...
									else if(data==3)
									{
										swal({
										  ...
										  window.setTimeout(function()
										    { 
										      window.location.href= "dashboard.php?page=supplier";
										 	} ,1500);
									}
									else if(data==2)
									{
