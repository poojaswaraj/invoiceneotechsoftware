...
<div class="col-lg-12" id="filter"> 
	...
	<form id="frm_requisition" name="frm_requisition" method="post">
		...
		<label>Quantity <span class="required_field">*</span></label>
			<input type="number" class="form-control" name="txt_qty" id="txt_qty" placeholder="Quantity" value="<?php echo $data['qty']; ?>" required="" step=".01">
		</div>
		
		<div class="col-md-3">
		<label>Required By <span class="required_field">*</span></label>
			<input type="date" class="form-control" name="requried_by_date" id="requried_by_date" value="<?php echo $data['requried_by_date'] ?>" placeholder="Required By" required="">
		</div>

		<div class="panel-body"></div>
		<div class="col-md-6">
		<label>Comment</label>
			<textarea class="form-control" name="txt_comment" id="txt_comment" placeholder="Comment"><?php echo $data['comment']; ?></textarea>
		</div>
		<?php if($utype=="admin"){ ?>
		<div class="col-md-3">
		<label>Approval <span class="required_field">*</span></label>
			...
			if(!empty($rec_id))
					{
				$sq = mysql_query("SELECT * FROM daily_material_requisition WHERE id='$rec_id'")or die(mysql_error($connection));
				$ar = mysql_fetch_array($sq);
			?>
				<option value="<?php echo $ar['approval']; ?>"><?php echo $ar['approval']; ?></option>
		  ...
		  <?php 
			...
			<tr id="invoice-23">
			  ...
			  <?php if($utype=='admin'){ ?>
			  	...
			  	<?php }else{ ?>
			  	<a href='user_dashboard.php?page=material_requisition&id=<?php echo $id; ?>' title='Edit Details' data-toggle='tooltip'><span class="icon fa fa-edit"></span></a>
			  	<?php } ?> |
