...
<input type="hidden" name="txt_id" id="txt_id" value="<?php echo $rec_id; ?>">
...
<div class="col-lg-9">
	...
	<div class="col-md-3">
		...
		<input type="text" class="form-control" name="supp_pan" id="supp_pan" placeholder="Supplier PAN No." readonly="" value="<?php if(isset($rec_id)){echo $data['panno'];}else{}  ?>">
	</div>
	<div class="col-md-3">
		<label>Supplier GST No</label>
		<input type="text" class="form-control" name="txt_gstno" id="txt_gstno" placeholder="Supplier GST No" readonly="" value="<?php if(isset($rec_id)){echo $data['gst_no'];}else{}  ?>">
	</div>
	...
	<div class="col-md-12">
		<div class="col-md-6">
			...
			<input type="text" class="form-control" name="supp_comp" id="supp_comp" placeholder="Company Name" required="" readonly value="<?php if(isset($rec_id)){echo $data['comp_name'];}else{}  ?>">
		</div>
...
<div class="col-lg-3">
	...
	<div class="col-md-12">
		...
		<?php 
			if(!empty($rec_id))
			{	
			  $sq = mysql_query("SELECT b.* FROM tbl_po a INNER JOIN new_project b ON a.pro_id=b.id WHERE a.id='$rec_id'")or die(mysql_error($connection));
			$arr=mysql_fetch_array($sq);
  ...
  <div class="panel panel-default">
    <div style="height: 200px; overflow-y: scroll; overflow-x: hidden;">
	  ...
	  <?php
	  	...
	  	$sq = mysql_query("SELECT b.pro_name,c.mat_name,c.brand_name, a.* FROM `tbl_po_matrials` a INNER JOIN new_project b ON a.pro_id=b.id INNER JOIN mst_material c ON a.mate_id=c.id WHERE a.po_id='$rec_id'")or die(mysql_error($connection));
            ...
            <form  method="post" id="edit_myform" enctype="multipart/formdata"> 
                ...
                <div class="col-md-12">
					<div class="col-md-6">
						...
						<input type="number" class="form-control" name="txt_rates" id="txt_rates" placeholder="Rate" required="" onkeyup="taxoncalculations();">
					</div>
...
//to edit material data 
$(document).ready(function(){
    $('#mate_name').on('change',function(){
        var id = $(this).val();
      ...
      if(id)
        {
          $.ajax({
              ...
              $('#mate_name').html('Select Material first'); 
          }
      });
});
		...
		$.ajax({
			...
			if(document.pressed == 'add_item')
	  	{
			...
			$.ajax({
					data:$("#myform").serialize(),
					type:"POST",
					url:'insert_editpo_material.php',
					success: function(data)
					{
