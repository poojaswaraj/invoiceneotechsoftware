<?php
    if(isset($_GET["id"]))
    {
        ...
        $query = mysql_query("SELECT h.name as pm,h.email as pm_email,h.mobile,a.pro_id as pro_id,a.req_id as req_id,c.name,c.email,c.contact,c.panno,c.gst_no,c.comp_name,c.state,c.city,c.address,b.pro_name,b.address as pro_address,s.name as State,s.state_code,f.ct_name,g.name as pro_state,g.state_code as pro_st_code, e.* FROM `tbl_po` a LEFT JOIN daily_material_requisition d ON d.id=a.req_id LEFT JOIN new_project b ON a.pro_id=b.id OR d.pro_id=b.id INNER JOIN tbl_debit e ON a.id=e.po_id INNER JOIN mst_supplier c ON e.supp_id=c.id INNER JOIN tbl_states s ON s.id=c.state INNER JOIN city f ON f.ct_id=c.city INNER JOIN tbl_states g ON b.state=g.id INNER JOIN project_team k ON k.pro_id=b.id INNER JOIN user_profile h ON h.id=k.emp_id WHERE e.id='$id'")or die(mysql_error($connection));
            ...
            <!---------------------------------------->
              <?php 
               ...
               ?>
                ...
                <div class="div-left1">
                    <div class="header">DEBIT NOTE </div>
                ...
                <div class="outter-block">
                    <div class="div-left" style="width:49%;">
						...
						<div class="block2">
                            DEBIT No: <b><?php echo $data['debit_no']; ?></b>
                        </div>
                        <div class="block2">
                            Date Of Issue: <b>
                            <?php echo date('d-M-Y',strtotime($data['debit_date']));?></b>
                        </div>
                ...
                <div class="outter-block">
                   <table>
                    ...
                    <?php 
                    $sr_no=0;
                    $sq = mysql_query("SELECT t.state_code, m.mat_name,m.brand_name,g.* FROM  `tbl_po` a  LEFT JOIN daily_material_requisition d ON d.id=a.req_id LEFT JOIN new_project b ON a.pro_id=b.id OR d.pro_id=b.id LEFT JOIN tbl_debit e ON a.id=e.po_id INNER JOIN mst_supplier c ON e.supp_id=c.id INNER JOIN tbl_debit_materials g ON e.id=g.debit_id INNER JOIN mst_material m ON g.mate_id=m.id INNER JOIN tbl_states t ON t.id=c.state WHERE e.id='$id'")or die(mysql_error($connection));
                ...
                <div class="outter-block">
                   <?php
                    $ss = mysql_query("SELECT SUM(total_amt) as total FROM `tbl_debit_materials` WHERE debit_id='$id'")or die(mysql_error($connection));
