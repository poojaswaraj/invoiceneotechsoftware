<?php
    if(isset($_GET["id"]))
    {
        ...
        $query = mysql_query("SELECT f.name as pm,f.email as pm_email,f.mobile,e.pro_name,e.address as pro_address,g.name as pro_state,g.state_code as pro_st_code,c.name as State,c.state_code,d.ct_name,b.name,b.email,b.contact,b.panno,b.gst_no,b.comp_name,b.state,b.city,b.address,p.* FROM tbl_po p INNER JOIn `daily_material_requisition` a ON p.req_id=a.id INNER JOIN mst_supplier b ON p.supp_id=b.id INNER JOIN tbl_states c ON c.id=b.state INNER JOIN city d ON d.ct_id=b.city INNER JOIN new_project e ON e.id=a.pro_id INNER JOIN user_profile f ON f.id=a.requ_made_by INNER JOIN tbl_states g ON e.state=g.id WHERE p.id='$id'")or die(mysql_error($connection));
               ...
               ?>
                ...
                <div class="outter-block">
                   <table>
                    ...
                    <tbody>
                    <?php 
                    $sq = mysql_query("SELECT t.state_code,d.name,b.pro_name,c.mat_name,c.brand_name,g.unit as po_unit, g.* FROM `tbl_po` h INNER JOIN `daily_material_requisition` a ON h.req_id=a.id INNER JOIN new_project b ON a.pro_id=b.id INNER JOIN mst_material c ON a.mate_id=c.id INNER JOIN mst_supplier d ON d.id=h.supp_id INNER JOIN tbl_states t ON t.id=d.state INNER JOIN tbl_po_matrials g ON h.id=g.po_id WHERE h.id='$id'")or die(mysql_error($connection));
                   ...
                   <?php
                    ...
                    <table style="font-size:10px;">
                        <tr> 
                            ...
                            <td> <b><?php echo $data['grand_total']; ?></b></td>
                            </tr>
                        </table>
                    </div>
                </div>
